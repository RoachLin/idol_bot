...
def print_hello_world():
    ...
    member_list = [
        ...
        "尾木 波菜（≠ME）", "落合 希来里（≠ME）", "蟹沢 萌子（≠ME）", "河口 夏音（≠ME）", "川中子 奈月心（≠ME）", "櫻井 もも（≠ME）",
        "菅波 美玲（≠ME）", "鈴木 瞳美（≠ME）", "谷崎 早耶（≠ME）", "冨田 菜々風（≠ME）", "永田 詩央里（≠ME）", "本田 珠由記（≠ME）",
        "逢田 珠里依（≒JOY）", "天野 香乃愛（≒JOY）", "市原 愛弓（≒JOY）", "江角 怜音（≒JOY）", "大信田 美月（≒JOY）", "大西 葵（≒JOY）",
    ...
    while True:
        ...
        if send == "":
            pyautogui.click(x=2666, y=1777)  # 如果待发送内容为空，点击屏幕，防止手机熄屏
        else:
            date = datetime.now()
